...
import { ScanLine, CheckCircle2, CircleAlert, LoaderCircle, Plus, X } from "lucide-react";
...
function CamerasPageContent() {
  ...
  const connection = statusQuery.isLoading
    ? { label: t("status.loading"), tone: "loading" as const, status: "loading" as const }
    : statusQuery.isError
      ? { label: t("status.unavailable"), tone: "offline" as const, status: "unavailable" as const }
      : statusQuery.data?.system?.running === false
        ? { label: t("status.degraded"), tone: "offline" as const, status: "degraded" as const }
        : { label: t("status.connected"), tone: "connected" as const, status: "connected" as const };
  const cameras = camerasQuery.data?.cameras ?? [];
  ...
  const updateWorkspace = (next: { cameraId?: string; view?: CameraWallView }) => {
    ...
    router.replace(`/${locale}/cameras${params.size ? `?${params.toString()}` : ""}`, { scroll: false });
  };

  return (
    <AppShell>
      <section className="camera-workspace mx-auto w-full max-w-[1800px] px-4 py-4 sm:px-6 lg:px-8">
        <div className="camera-page-header">
          <div className="flex min-w-0 items-center gap-3">
            <span className="camera-title-icon"><ScanLine className="h-4 w-4" aria-hidden /></span>
            <h1 className="text-xl font-medium tracking-tight text-white">{t("title")}</h1>
            <span className="camera-count-label" aria-label={t("workspace.cameraCount", { count: cameras.length })}>{camerasQuery.isLoading || camerasQuery.isError ? "?" : cameras.length}</span>
          </div>
          <div className="camera-header-actions flex items-center gap-3">
            <SystemConnectionIndicator tone={connection.tone} label={connection.label} />
            <Button type="button" className="min-h-9 self-start px-3 text-sm" onClick={() => setRegisterOpen(true)}>
        ...
        <div className="mt-3">
          ...
          {!camerasQuery.isLoading && !camerasQuery.isError && cameras.length > 0 ? <CameraWall key={`${view}-${activeCameraId}`} cameras={cameras} events={eventsQuery.data?.events ?? []} initialView={view} initialCameraId={activeCameraId} systemStatus={connection.status} eventsUnavailable={eventsQuery.isError} onViewChange={(nextView) => updateWorkspace({ view: nextView })} onCameraChange={(cameraId) => updateWorkspace({ cameraId, view: "focus" })} /> : null}
        </div>
          ...
          <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Register camera source">
            ...
            <aside className="glass-panel absolute right-0 top-0 h-full w-full max-w-xl overflow-y-auto border-y-0 border-r-0 p-5 shadow-2xl">
              <div className="mb-4 flex items-center justify-between gap-3">
                <div>
                  <p className="text-xs uppercase tracking-[0.14em] text-slate-500">Camera setup</p>
                  <h2 className="mt-1 text-xl font-semibold text-white">Add Camera</h2>
                </div>
...
function SystemConnectionIndicator({ tone, label }: { tone: "loading" | "offline" | "connected"; label: string }) {
  ...
  return (
    <span className={`inline-flex min-h-9 items-center gap-2 rounded-md px-2 text-xs ${tone === "connected" ? "text-emerald-200" : tone === "offline" ? "text-rose-100" : "text-amber-100"}`} role="status">
